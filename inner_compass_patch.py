...
file_path = "src/components/InnerCompass.tsx"
with open(file_path, "r", encoding="utf-8") as f:
    content = f.read()

# 1. Add useRouter import and tertiaryData
import_replacement = """import { cn } from "@/lib/utils";
...
const tertiaryData: Record<string, TertiaryData> = {
  // ANGRY subtree
  betrayed: {
...
content = content.replace('import { cn } from "@/lib/utils";', import_replacement)

# 2. Add router hook
hook_replacement = """export default function InnerCompass() {
  const router = useRouter();
  const [viewState, setViewState] = useState<ViewState>("primary");"""

content = content.replace('export default function InnerCompass() {\n  const [viewState, setViewState] = useState<ViewState>("primary");', hook_replacement)

# 3. Replace subtitle text
content = content.replace("What&apos;s here right now?", "What do you feel right now?")

# 4. Replace renderComplete function
old_render_complete = """  const renderComplete = () => {
...
new_render_complete = """  const renderComplete = () => {
    if (!selectedTertiary || !selectedPrimary) return null;

    const data = tertiaryData[selectedTertiary] || {
      affirmation: "You noticed it — that is enough. Every emotion has something to say, and you listened.",
    ...
    return (
      <div className={cn(
        "w-full flex flex-col items-center justify-center px-4 py-8",
        emotionBg[selectedPrimary]
      )}>
        <div className="max-w-lg w-full space-y-5">
          ...
          <div className="flex justify-center">
            <div className={cn(
              ...
              "backdrop-blur-sm border border-border/20 text-foreground/90"
            )}>
          ...
          {/* Affirmation card */}
          <div className="rounded-3xl p-6 text-center bg-background/20 backdrop-blur-md border border-border/20">
          ...
          <div className="rounded-2xl p-5 bg-background/15 backdrop-blur-md border border-border/20">
            ...
            <div className="flex flex-col gap-2">
              {data.modules.map((mod) => (
                <button
                  ...
                  className={cn(
                    "flex items-center gap-3 rounded-xl px-4 py-3 text-left w-full",
                    "bg-gradient-to-r",
                    emotionColors[selectedPrimary],
          ...
          <div className="text-center">
            <button
              onClick={handleReset}
              className="text-sm text-muted-foreground hover:text-foreground transition-colors underline bg-transparent border-none"
            >
              Check in again
            </button>
          </div>
...
content = content.replace(old_render_complete, new_render_complete)
